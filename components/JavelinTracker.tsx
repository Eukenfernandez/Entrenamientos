...
interface JavelinTrackerProps {
  records: ThrowRecord[];
  onAddRecord: (record: Omit<ThrowRecord, 'id'>) => void;
  onDeleteRecord: (id: string) => void;
}

export const JavelinTracker: React.FC<JavelinTrackerProps> = ({ records, onAddRecord, onDeleteRecord }) => {
    ...
    <div className="h-full bg-neutral-950 p-6 md:p-10 overflow-y-auto">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-end mb-6">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Competiciones</h1>
            <p className="text-neutral-400">Historial de marcas oficiales en competición.</p>
          </div>
          <div className="bg-neutral-900 px-6 py-3 rounded-xl border border-yellow-500/20">
            <span className="block text-xs text-neutral-400 uppercase tracking-wider">Mejor Marca (PB)</span>
            <span className="text-3xl font-bold text-yellow-500">{bestThrow} m</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-neutral-900 p-6 rounded-2xl border border-neutral-800 h-fit">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs text-neutral-400 mb-1">Marca (metros)</label>
                <input 
                  type="number" 
                  step="0.01"
                  value={distance}
